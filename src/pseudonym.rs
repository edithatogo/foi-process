...
pub fn pseudonymize_case_id(key: &[u8], source_case_id: &str) -> StableId {
    ...
    StableId::content("foi-process:confidential-case", &digest)
}
...
mod tests {
    ...
    fn pseudonyms_are_stable_but_key_scoped() {
        let first = pseudonymize_case_id(b"test-only-secret", "internal-case-42");
        assert_eq!(
            first,
            pseudonymize_case_id(b"test-only-secret", "internal-case-42")
        );
        assert_ne!(
            first,
            pseudonymize_case_id(b"rotated-secret", "internal-case-42")
        );
