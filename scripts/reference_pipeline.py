...
def generate() -> None:
    ...
    for e in materialize(events):
        ...
        public_events.append({k:e[k] for k in ["event_id","logical_event_id","site","jurisdiction","case_id","activity","event_time","assertion_status"]} | {"evidence":links,"attributes":{k:v for k,v in e.get("attributes",{}).items() if k in {"authority_id","native_activity"}}})
