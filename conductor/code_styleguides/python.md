...
## 5. Modern Data & Performance Tooling Standards

### 5.1 Ruff Configuration & Rule Posture

- **Target Version:** Standardize on `target-version = "py311"` across all subprojects.
- **Rule Posture:** Prefer explicit, maximum strict lint rule selection arrays over wildcard `ALL` to maintain
  consistent rule posture across all workspace components.

### 5.2 Polars (DataFrames & Transformations)

- **Lazy Evaluation First:** Always prefer lazy evaluation (`pl.scan_parquet`, `pl.scan_csv`) over eager loading
  (`pl.read_parquet`, `pl.read_csv`) for large corpus transformations.
- **Streaming Execution:** Perform terminal collection using `.collect(streaming=True)` to process datasets that exceed available RAM.
- **Avoid Pure Python Loops:** Keep dataframe processing within Polars expressions and avoid mapping pure Python functions over rows.

### 5.3 Pydantic vs. msgspec Boundaries

- **Pydantic (Validation & Config):** Use Pydantic v2 exclusively at boundaries: config files, environment parser
  settings, inputs, and API request/response structures.
- **msgspec (High-Volume Records):** Use `msgspec.Struct` for high-performance pipeline record serialization and
  deserialization to minimize runtime overhead in hot paths.

**BE CONSISTENT.** When editing code, match the existing style.

*Source: [Google Python Style Guide](https://google.github.io/styleguide/pyguide.html) + Project Performance Consensus*
